...
lbl_pregunta = tk.Label(ventana, text="Cargando...", font=("Verdana", 25, "bold"), 
                        wraplength=400, justify="center", height=4)
lbl_pregunta.pack(pady=30)

# Contenedor para los botones (así quedan ordenaditos)
frame_respuestas = tk.Frame(ventana)
frame_respuestas.pack(pady=10)

# Botones con Lambda. Fíjate que ahora pasamos la letra Y el propio objeto botón
btn_a = tk.Button(frame_respuestas, text="", width=50, height=2, font=("Arial", 25),
                  command=lambda: comprobar("A", btn_a))
btn_a.pack(pady=5)

btn_b = tk.Button(frame_respuestas, text="", width=50, height=2, font=("Arial", 25),
                  command=lambda: comprobar("B", btn_b))
btn_b.pack(pady=5)

btn_c = tk.Button(frame_respuestas, text="", width=50, height=2, font=("Arial", 25),
                  command=lambda: comprobar("C", btn_c))
btn_c.pack(pady=5)

btn_d = tk.Button(frame_respuestas, text="", width=50, height=2, font=("Arial", 25),
                  command=lambda: comprobar("D", btn_d))
btn_d.pack(pady=5)

# Barra de estado inferior
lbl_status = tk.Label(ventana, text="", font=("Arial", 20, "italic"), fg="gray")
